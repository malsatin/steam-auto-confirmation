...
Bot.prototype.start = function() {
    log.info("Logging in as " + this._config.username);
    ...
    if(offer.itemsToGive.length > 0) {
        if(this._config.accept_incoming !== null) {
            if(this._config.accept_incoming) {
                log.log("Offer#" + offer.id + ": ok, accepting it.");

                this._acceptOffer(offer);
            } else {
                log.log("Offer#" + offer.id + ": ok, declining it.");
...
Bot.prototype.hasNoConfirmations = function(checkTime = 10 * 1000) {
    return Date.now() - this.lastConfirmedAt > checkTime;
};
